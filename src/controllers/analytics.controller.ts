...
export async function getAnalytics(_req: Request, res: Response) {
  ...
  const [overdueFirstResponse, overdueResolution, nearingDeadline, responseTiming, resolutionTiming] = await Promise.all([
    ComplaintModel.countDocuments({ firstResponseAt: { $exists: false }, firstResponseDueAt: { $lt: now }, status: { $nin: ['resolved', 'closed'] } }),
    ComplaintModel.countDocuments({ status: { $nin: ['resolved', 'closed'] }, resolutionDueAt: { $lt: now } }),
    ComplaintModel.countDocuments({ status: { $nin: ['resolved', 'closed'] }, resolutionDueAt: { $gte: now, $lte: new Date(now.getTime() + 24 * 60 * 60 * 1000) } }),
  ...
  for (const row of byCategory) categoryCounts[row._id] = row.count;
  const categoryStatusCounts = Object.fromEntries(complaintCategories.map(category => [category, { pending: 0, under_review: 0, resolved: 0, closed: 0 }]));
  for (const row of byCategoryStatus) {
    (categoryStatusCounts[row._id.category] as Record<string, number>)[row._id.status] = row.count;
  }
